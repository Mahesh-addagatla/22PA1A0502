...
import { urlStore, isExpired, cleanExpiredUrls } from "../db/memoryStore.js";
...
export const createShortURL = async (req, res) => {
  ...
  try {
    ...
    if (!url || !shortcode) {
      ...
      return res
        .status(400)
        .json({ message: "URL and shortcode are required" });
    }
    ...
    try {
      new URL(finalUrl); // now this will work
    } catch (urlError) {
    ...
    await Log(
      ...
      "info",
      "controller",
      `Shortened URL created: ${shortcode} (expires: ${expiryTime.toISOString()})`
    );

    return res.status(201).json({
      shortLink: `http://localhost:9000/api/${shortcode}`,
      expiry: expiryTime.toISOString(),
    });
  } catch (error) {
    await Log(
